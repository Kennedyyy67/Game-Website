...
if (!in_array($field, ['target_price', 'notes'])) {
    echo json_encode(['success' => false, 'error' => 'Invalid field.']);
    exit;
}

// 4. Instantiate and call WishlistManager
try {
    $wishlistManager = getWishlistManager();

    if ($field === 'target_price') {
        $value = $value === '' ? null : (float)$value;
        $result = $wishlistManager->updateTargetPrice($userId, $gameId, $value);
    } elseif ($field === 'notes') {
        $result = $wishlistManager->updateNotes($userId, $gameId, $value);
    }
